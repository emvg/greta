rule mdl_o_linger:
    ...
    resources:
        mem_mb=128000,#lambda wildcards, attempt: restart_mem(wildcards, attempt) * 2,
        runtime=(
...
rule mdl_o_linger_baseline:
    ...
    input:
        ...
        mdata=rules.extract_case.output.mdata
    output:
        dir=directory('dts/{org}/{dat}/cases/{case}/runs/linger_baseline/'),
        out='dts/{org}/{dat}/cases/{case}/runs/o_linger_baseline.o_linger_baseline.o_linger_baseline.o_linger_baseline.mdl.csv'
    params:
        version='baseline',
        mode=config['methods']['linger']['mode'],
        organism=lambda w: config['dts'][w.dat]['organism'],
