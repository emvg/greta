...
if __name__ == '__main__':
    ...
    print(f"[INFO] Importing fragment files...")
    ...
    del _
    print(f"[INFO] Fragment import complete")
    ...
    for sample_id, tmp_file in zip(sample_ids, tmp_files):
        ...
        # Process barcode names - remove any suffixes added by snapatac2, then add sample prefix
        tmp.obs.index = [barcode.split('-1')[0].replace('_atac_fragments', '') for barcode in tmp.obs.index]
        # Convert commas to dots to match annotation format (fragments use commas, celltype file uses dots)
        tmp.obs.index = [bc.replace(',', '.') for bc in tmp.obs.index]
        # Prepend sample name to match annotation format (sample_barcode)
        tmp.obs.index = [f"{sample_id}_{bc}" if not bc.startswith(sample_id) else bc for bc in tmp.obs.index]
        
        # Filter by annotation
        obs = pd.merge(tmp.obs, annot, left_index=True, right_index=True)
        
        if len(obs) == 0:
            raise ValueError(f"No matching cells found between fragments and annotation for sample {sample_id}")
    ...
    peaks = snap.tl.merge_peaks(atac.uns['macs3'], snap.genome.mm10)
