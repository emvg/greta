...
run_HGT <- function(GAS,result_dir,data_type,envPath=NULL,lr=NULL, epoch=NULL, n_hid=NULL, n_heads=NULL,cuda=0){
  ...
  list_in <- assign("list_in", list(lr=lr, epoch=epoch, n_hid=n_hid, n_heads=n_heads, result_dir=result_dir, cuda=cuda, data_type=data_type, cell_gene=GAS, gene_name=rownames(GAS), cell_name=colnames(GAS)), envir = .GlobalEnv) 
    py_run_string("import sys; sys.path.insert(0, 'workflow/scripts/mth/deepmaps')")
...
RI_cell <-
  function(obj,
           ...
           peak_TF,
           graph.out) {
    if (length(ct_regulon) == 0)
      return(matrix(0, nrow = 0, ncol = length(graph.out)))
    v <- vector()
    for (i in seq_len(length(ct_regulon))) {
      v <-
    ...
    for (j in (1:length(graph.out))) {
      ...
      cell_acc <- as.vector(peak_cell[, j])
      left  <- sweep(gene_peak[g, ], 2, cell_acc, "*")
      right <- sweep(peak_TF[, t],   1, cell_acc, "*")
      hhh   <- left %*% right
...
calRAS <- function(RI_C, ct_regulon, graph.out, TFinGAS = T) {
  if (length(ct_regulon) == 0 || nrow(RI_C) == 0) {
    empty_ras <- as(matrix(0, nrow = 0, ncol = 0), "sparseMatrix")
    empty_c   <- as(matrix(0, nrow = 0, ncol = length(graph.out)), "sparseMatrix")
    return(list(empty_ras, matrix(0, 0, 0), list(), empty_c))
  }
  ...
  for (i in seq_len(length(ct_regulon))) {
    ...
    RAS_C[tf, ] <-
      colMeans(RAS_E[paste(tf, ct_regulon[[i]], sep = "_"), , drop = FALSE])
  }
...
masterFac <- function(ct_regulon, RI_CT) {
  if (length(ct_regulon) == 0) return(list(list(), list(), list()))
  TF_CT <- unlist(strsplit(names(ct_regulon), split = "_"))
  if (length(TF_CT) == 0) return(list(list(), list(), list()))
  ...
  for (i in (CT)) {
    if (length(ct_regulon[CR_R == i]) == 0) next
    TF_CT <- unlist(strsplit(names(ct_regulon[CR_R == i]), split = "_"))
    if (length(TF_CT) == 0) next
    ...
    for (k in seq_len(length(ct_regulon[CR_R == i]))) {
...
CalRAS2 <- function(ct_regulon, graph.out) {
  if (length(ct_regulon) == 0)
    return(as(matrix(0, nrow = 0, ncol = length(graph.out)), "sparseMatrix"))
  RAS_C2 <-
...
inp<-function(GAS = GAS, att , cell_hgt_matrix, gene_hgt_matrix, wdf1=1, thdf2=0.5, l=1.2){
